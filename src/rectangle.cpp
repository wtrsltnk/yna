...
bool Rectangle::Contains(
    const struct Point &point) const
{
...
{
    return !(
        (rectangle.X < this->X) || (rectangle.Y < this->Y) || (rectangle.Width > (this->Width - (rectangle.X - this->X))) || (rectangle.Height > (this->Height - (rectangle.Y - this->Y))));
}

// Pushes the edges of the Rectangle out by the horizontal and vertical values specified.
void Rectangle::Inflate(
...
Rectangle Rectangle::Intersect(
    const Rectangle &rectanlge1,
    const Rectangle &rectanlge2)
{
    return Rectangle();
}
